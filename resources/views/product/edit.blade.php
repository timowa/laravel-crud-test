    <div class="sm:container px-3 py-7 containerModal" x-show="openEditModal"  x-transition:enter="transition ease-out duration-300"
        ...
        <div class="flex justify-between">
            <h1 class="text-white font-bold">Редактировать <strong x-text="p_name"></strong></h1>
            <a href="#"  @click=" openEditModal = false">
        ...
        <form method="put" x-bind:action="'product' + '/' + p_id" class="w-3/4" id="formEdit" >
            @csrf
            @if(auth()->user()->is_admin)
                <div class="form-group">
                    <label for="article" >Артикул</label>
                    <input type="text" name="article" id="article" x-model="p_article">
                </div>
            @endif
            <div class="form-group">
                <label for="name">Название</label>
                <input type="text" name="name" id="name" x-model="p_name">
            </div>
            <div class="form-group">
                <label for="status">Статус</label>
                <select id="status" class="text-black py-0" name="status" x-model="p_available">
            ...
            <template x-for="attr in p_data">
                <div class="flex justify-between items-center">
                        <div class="form-group w-full pr-2.5">
                            <label for="">Название</label>
                            <input type="text" name="attrName[]" x-model="attr.name">
                        </div>
                        <div class="form-group w-full pr-2.5">
                            <label for="">Значение</label>
                            <input type="text" name="attrValue[]" x-model="attr.value">
                        </div>
                        <a href="#" class="deleteRow">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="11" viewBox="0 0 10 11" fill="none" style="min-inline-size: min-content">
