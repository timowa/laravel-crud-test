$('.attributes').on('click','.deleteRow',function(e){
    e.preventDefault();
    $(this).parent().remove();
})

function reloadTable(){
    $.get( $(location).attr('href'), {}, function(results){
        $('table').html($(results).find("table").html())
    });
}

$('button.btn-submit').on('click',formSubmit);
...
$('.addAttr').on('click',function(e){
    e.preventDefault();
    console.log(1)
    $(this).parent().find('.attributes').append('<div class="flex justify-between items-center">'+
